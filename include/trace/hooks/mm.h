...
DECLARE_HOOK(android_vh_io_statistics,
	TP_PROTO(struct address_space *mapping, pgoff_t index,
		 unsigned int nr_pages, bool is_read, bool is_dio),
	TP_ARGS(mapping, index, nr_pages, is_read, is_dio));
DECLARE_HOOK(android_vh_should_fault_around,
	TP_PROTO(struct vm_fault *vmf, bool *should_fault_around),
	TP_ARGS(vmf, should_fault_around));
